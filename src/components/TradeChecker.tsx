  ...
  return (
    <div className="mt-1 space-y-1">
      ...
      <div className="flex flex-wrap items-center gap-1">
        ...
        {outlook && <Tooltip text="Average per-season value over the NEXT 3 seasons (this one excluded), on the Now scale. MLB: his current healthy value walked along the aging curve × the odds his career is still going. Prospects: the peak-season estimate bent by the maturation ramp and arrival/bust risk — a teenager gives you little in the NEXT three years even when the peak is big."><span className={`${chip} bg-violet-500/25 text-violet-200`}>3yr {outlook.next3.toFixed(1)}</span></Tooltip>}
        {outlook && <Tooltip text="His best single season AFTER this one, on the Now scale. MLB: slightly above Now for a pre-peak guy, below it for a declining vet. Prospects: his projected PEAK season (wRC+/HR/SB/WAR at an everyday MLB role, peak age) priced like an MLB player's Now — the ceiling if he develops as projected; bust risk is in Keep, not here."><span className={`${chip} bg-rose-500/25 text-rose-200`}>Best yr {outlook.peakSeason.toFixed(1)}</span></Tooltip>}
      </div>
      {/* ability */}
      <div className="flex flex-wrap items-center gap-1">
